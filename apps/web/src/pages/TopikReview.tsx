    ...
    <div className="app-page max-w-[800px]">
      ...
      <section className="mt-7 border-b border-[var(--border)] pb-7" aria-labelledby="topik-owner-fsrs-title">
        ...
        {card && <article className="surface-panel mt-4 p-5 sm:p-6">
          ...
          {card.item.audio?.kind === 'google' && <button type="button" onClick={() => audio.play(card.item.audio!, { contentType: 'topik_owner_item', contentId: card.item.id, levelTag: String(card.item.target_grade), section: card.item.item_type })} className="mt-4 touch-target rounded-[var(--radius-md)] bg-[var(--accent)] px-4 font-bold text-white">Google 한국어 음성 재생</button>}
