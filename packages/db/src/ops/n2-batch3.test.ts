...
import {
  ...
  N2_BATCH_3_KANJI,
  N2_BATCH_3_PATH,
  N2_BATCH_3_SOURCE_ASSET_ID,
  n2Batch3ContentRowsSql,
} from '../seed/n2-batch3.js';

test('N2 Batch 3 adds self-authored health, community, and sustainability study data', () => {
  const plan = buildN2Batch3Plan();
  ...
  assert.match(n2Batch3ContentRowsSql(), /reading_questions/);
  for (const char of N2_BATCH_3_KANJI) assert.match(sql, new RegExp(`'${char}'`));
});
