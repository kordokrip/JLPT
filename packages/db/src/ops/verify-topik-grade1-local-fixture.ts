...
const checks = [
  ...
  { name: 'TOPIK R2-only pending bindings', expected: fixture.counts.audioBindings, sql: `SELECT count(*) AS count FROM content_audio_bindings b JOIN learning_content_stable_refs r ON r.stable_ref = b.stable_ref WHERE r.source_asset_id = ${assetId} AND b.binding_state = 'preparing' AND b.asset_id IS NULL AND length(trim(COALESCE(b.unavailable_reason, ''))) > 0` },
  { name: 'vocabulary uses pronunciation and listening uses listening role', expected: 2, sql: `SELECT count(*) AS count FROM topik_owner_authored_curriculum_items i JOIN content_audio_bindings b ON b.item_type = 'topik-owner-item' AND b.item_id = i.id AND b.language = 'ko' WHERE i.unit_id = ${literal(fixture.unitId)} AND b.audio_role = CASE WHEN i.item_type = 'listening' THEN 'listening' ELSE 'pronunciation' END` },
