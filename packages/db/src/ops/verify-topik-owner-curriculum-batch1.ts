...
const checks = [
  ...
  { name: 'TOPIK Batch 1 pending R2 bindings', expected: manifest.counts.audioBindings, sql: `SELECT count(*) AS count FROM content_audio_bindings b JOIN learning_content_stable_refs r ON r.stable_ref = b.stable_ref WHERE r.source_asset_id = ${asset} AND b.language = 'ko' AND b.binding_state = 'preparing' AND b.asset_id IS NULL` },
