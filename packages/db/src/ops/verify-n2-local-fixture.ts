...
const sqlChecks: Array<{ name: string; expected: number; sql: string }> = [
  ...
  { name: 'N2 Google speech bindings', expected: fixture.counts.audioBindings, sql: `SELECT count(*) AS count FROM content_speech_bindings b JOIN learning_content_stable_refs r ON r.stable_ref = b.stable_ref WHERE r.learning_track = 'jlpt-ja' AND r.level_tag = 'N2' AND r.source_asset_id = ${assetId} AND b.provider = 'google-browser' AND b.binding_state = 'ready'` },
  { name: 'N2 speech bindings match stable item identity', expected: 0, sql: `SELECT count(*) AS count FROM content_speech_bindings b JOIN learning_content_stable_refs r ON r.stable_ref = b.stable_ref WHERE b.item_type <> r.item_type OR b.item_id <> r.item_id` },
