...
function jlptChecks(items: readonly JlptN3PracticeDraft[]) {
  ...
  return [
    ...
    { name: 'JLPT browser-speech listening', passed: listening.every((item) => item.speech_provider === 'google-browser' && Boolean(item.audio_script_ja?.trim())) && kanji.every((item) => item.speech_provider === 'unavailable' && item.audio_script_ja === null), detail: 'listening uses Japanese text with Google-preferred same-language browser speech; kanji has no audio asset' },
  ];
}
...
function topikChecks(items: readonly TopikOwnerBatch5Draft[]) {
  ...
  return [
    ...
    { name: 'TOPIK browser-speech text', passed: listening.every((item) => item.speech_provider === 'google-browser' && Boolean(item.audio_text_ko?.trim())) && items.filter((item) => item.section !== 'listening').every((item) => item.speech_provider === 'unavailable' && item.audio_text_ko === null) && items.every((item) => !item.audio_text_ko || !/r2:\/\/|audio_r2_key/iu.test(item.audio_text_ko)), detail: 'only listening has Korean Google-preferred browser speech text; non-listening has no speech binding source' },
