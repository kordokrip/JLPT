...
export function buildTopikGrade1LocalFixturePlan(): TopikGrade1LocalFixturePlan {
  ...
  const statements = [
    ...
    [
      ...
      `  '안녕하세요는 처음 만나거나 정중하게 인사할 때 쓸 수 있습니다.', '「안녕하세요」は初対面や丁寧にあいさつするときに使えます。', '안녕하세요 can be used when first meeting someone or greeting politely.', 1, '안녕하세요.', ${esc(TOPIK_GRADE1_LOCAL_FIXTURE_SOURCE_ASSET_ID)});`,
    ].join('\n'),
    [
      'INSERT OR IGNORE INTO `topik_owner_authored_curriculum_items`',
      '  (`id`, `unit_id`, `target_grade`, `stable_ref`, `item_type`, `prompt_ko`, `prompt_ja`, `prompt_en`, `answer_json`, `explanation_ko`, `explanation_ja`, `explanation_en`, `audio_required`, `audio_text_ko`, `source_asset_id`)',
