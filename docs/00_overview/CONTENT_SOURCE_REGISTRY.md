...
기준일: 2026-08-09 KST. 이 레지스트리는 후보를 기록할 뿐 현재 seed에 external data를 승인하거나 적재하지 않는다.

| 후보 | 허용 후보 용도 | seed 등록 전 필수 증적 | 현재 상태 |
| --- | --- | --- | --- |
| JMdict / KANJIDIC2 | 표기·읽기·의미의 후보 검토 | exact 배포본 URL, version, hash, attribution, license review | 후보만 등록 |
| KanjiVG | 획순 SVG 검토 | file URL, creator, license, hash, derivative/표시 계획 | 후보만 등록 |
| 한국어기초사전 Open API | 어휘·정의·발음 표기 검토 | API terms 재확인, API key 관리, response hash, attribution | 후보만 등록 |
...
| 자체 저작 | 학습 본문·예문·문제·해설·대본 | source text hash, source asset ID, stable refs | 운영 중 |
