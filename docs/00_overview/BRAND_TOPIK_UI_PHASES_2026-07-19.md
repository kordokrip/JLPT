...
| 6 Preview QA·수동 출시           | 자동 QA 완료·승인 대기 | 전용 Preview D1/Worker/Pages, 사용자 디자인 QA, JLPT 운영 회귀, 사람 승인 |
...
## 8. Phase 6 원격 Preview 검증

Preview Worker `nihongo-n3-api-topik-preview`를 전용 D1과 연결하고 Pages branch alias
`https://feature-topik-product-expans.nihongo-n3.pages.dev`의 `API_ORIGIN`을 해당 Worker로
설정했다. Preview Worker는 production cron을 상속하지 않도록 빈 trigger 목록을 명시했고,
Cloudflare API에서 schedule 0건을 다시 확인했다. Google OAuth는 별도 preview callback과
secret 승인 전까지 비활성 상태다.

| 원격 관문              | 결과                                                                                                            |
| ---------------------- | --------------------------------------------------------------------------------------------------------------- |
| Worker release         | `a1cf37580fd9d701564be1b71394b2b54811a4ac`, environment `preview`, maintenance `off`                            |
| track status           | `topik-ko`, `placement-preview`, listening/reading 공개, write enabled                                          |
| same-origin 계정 smoke | register 201, session 200, track patch 200, logout 후 anonymous session 확인                                    |
| Placement smoke        | 시작 201, 24문항(12/12), wire 정답·해설 노출 0, 제출/latest/review 200                                          |
| Preview cleanup        | 일회용 QA 계정과 cascade 데이터 삭제 후 잔존 계정 0                                                             |
| Dependency Audit       | [run 29691389462](https://github.com/kordokrip/JLPT/actions/runs/29691389462) 성공                              |
| CodeQL                 | [run 29691389460](https://github.com/kordokrip/JLPT/actions/runs/29691389460) 및 신규 경고 gate 성공            |
| Required Verification  | [run 29691389493](https://github.com/kordokrip/JLPT/actions/runs/29691389493) 성공                              |
| Fresh D1               | [run 29691389496](https://github.com/kordokrip/JLPT/actions/runs/29691389496) 성공, production change skip      |
| Chromium/WebKit        | [run 29691389463](https://github.com/kordokrip/JLPT/actions/runs/29691389463) 성공                              |
| Pages Preview          | [run 29691389479](https://github.com/kordokrip/JLPT/actions/runs/29691389479) build·deploy·isolation smoke 성공 |

첫 원격 CodeQL 실행은 Preview seed SQL을 PID 기반 임시 경로에 쓰는 코드를 High로
차단했다. 전용 `mkdtempSync` 디렉터리와 배타적 `wx` 생성을 사용하도록 수정했고, 위 SHA의
GitHub Advanced Security 신규 경고 gate가 성공한 것을 확인했다. required check를
optional로 바꾸거나 workflow를 우회하지 않았다.

Phase 0~5와 Phase 6 자동 검증은 완료됐다. 남은 관문은 Preview의 실제 디자인·콘텐츠 사용자
승인과 production 수동 출시 승인이다. 이 승인 전에는 `main` 병합, production Worker/Pages,
production D1/R2 변경을 수행하지 않는다.
