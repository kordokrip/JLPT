...
기준일: 2026-09-07 KST. 이 디렉터리는 학습 원본과 이를 운영하는 기준 문서다. 현재 실행 결과·코드·schema가 과거 서술보다 우선한다. `0028`과 새 학습 UX는 Production에 반영했고 기존 콘텐츠는 재시드하지 않았다. 현재 배포 ID·사후 검증은 CURRENT_STATE와 릴리스 원장, 날짜별 과거 실패는 오류 원장에 구분한다.
