...
test.describe('자연 일본어 검색 UX', () => {
  ...
  test.describe('translation response fixture', () => {
    // Only this mock UI regression blocks SW routing bypass (INC-QA-052).
    // The sidebar test keeps SW enabled; fixture output is not provider evidence.
    test.use({ serviceWorkers: 'block' });

    test('한국어 표현을 자연 일본어로 바꿔 어휘 검색에 사용한다', async ({ page }) => {
      let translationsIntercepted = 0;
      await ensureAuthenticated(page);
      await page.route('**/api/v1/ai/translate', async (route) => {
        translationsIntercepted++;
        expect(route.request().method()).toBe('POST');
        expect(route.request().postDataJSON()).toEqual({
          text: '오늘은 조금 피곤해요', target: 'ja', tone: 'polite',
        });
        await route.fulfill({
          status: 200,
      ...
      await expect.poll(() => translationsIntercepted, {
        message: 'The translation fixture must intercept exactly one request; live provider output is not this UI fixture',
      }).toBe(1);
