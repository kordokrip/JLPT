...
if [[ "${E2E_SKIP_DB_BOOTSTRAP:-}" != "1" ]]; then
  if pnpm -C apps/api exec wrangler d1 execute nihongo-n3-prod --local --config wrangler.toml --command "SELECT COUNT(*) FROM vocab; SELECT COUNT(*) FROM reading_passages; SELECT COUNT(*) FROM push_subscriptions;" >/dev/null 2>&1; then
